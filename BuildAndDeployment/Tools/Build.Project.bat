...
SET minorVersion="8.7-alpha"
SET majorVersion="0"

SET projectID=Viki.LoadRunner.Tools
SET project1="..\\..\\src\\%projectID%\\%projectID%.csproj"

"C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\MSBuild\15.0\Bin\MSBuild.exe" "%project1%" /verbosity:m /target:Rebuild /tv:15.0 /p:GenerateBuildInfoConfigFile=false /p:VisualStudioVersion=15.0 /p:platform=AnyCPU /p:TargetFrameworkVersion="v4.5" /p:Configuration=Release /p:OutputPath="%cd%\pack\lib\net45" /p:DebugSymbols=false /p:DebugType=none /P:SignAssembly=False /p:DocumentationFile=%projectID%.xml
