...
namespace Viki.LoadRunner.Playground.RnD
{
    public class PercentileMetricWithCount : IMetric
    {
        ...
        public PercentileMetricWithCount(params double[] percentiles)
            : this(percentiles, new string[] { })
        {
        }
        public PercentileMetricWithCount(double[] percentiles, string[] ignoredCheckpoints)
        {
            if (percentiles == null)
        ...
        {
            return new PercentileMetricWithCount(_percentiles, _ignoredCheckpoints)
            {
                Formatter = Formatter
            };
        }
            ...
            if (!_percentileValueCacheValid)
            {
                foreach (KeyValuePair<string, List<double>> pair in _row)
                {
                    ...
                    foreach (double targetPercentile in _percentiles)
                    {
                        ...
                        string countKey = string.Concat("Count of ", targetPercentile * 100, "%: ", pair.Key);
                        _percentileCache[countKey] = CalculateIndex(sortedData, targetPercentile) + 1;
                    }
                }
        ...
        private static int CalculateIndex(double[] sortedData, double percentile)
        {
            double realIndex = percentile * (sortedData.Length - 1);
            int index = (int)realIndex;
            return index;
